...
|---------|------|---------|----------|-------------|:-------:|-------------|
| [pg_timetable](/pg_timetable) | `bin` |  |  |  |  | Advanced scheduling for PostgreSQL |
...
Install `pg_timetable` via [Pigsty](https://pigsty.cc/docs/pgext/usage/install/) playbook:

```bash
./pgsql.yml -t pg_extension -e '{"pg_extensions": ["pg_timetable"]}'
```
